...
def show_page(i):
    ...
    with gallery_out:
        ...
        print(f"Showing {start+1} → {end} / {num_images}")
...
def open_crop_ui(path):
    ...
    with crop_out:
        ...
        confirm=widgets.Button(description="🎯 SET FACE", button_style="success")
        ...
        def set_ref(_):
            ...
            print("\n🎯 REFERENCE FACE SET — RUN MATCHING NOW\n")
...
def run_matching(_):
    ...
    with run_out:
        ...
        if ref_embedding is None:
            print("❗ No reference face selected.")
            return

        targets = ALL_IMAGES
        total=len(targets)

        print(f"🔍 Matching against {total} pictures")
        results=[]
        ...
        print("\n✅ DONE")
        print("📄 CSV:",csv_path)
        print("🎯 MATCHED:",df['is_match'].sum(),"/",len(df))
...
label=widgets.Label("📁 Enter export folder name:")
name_box=widgets.Text(value="matched_photos")

export_btn=widgets.Button(description="📂 EXPORT", button_style="info")
...
def export_matches(_):
    ...
    with export_out:
        ...
        if folder=="": print("❗ Empty name"); return

        csv_path=os.path.join(OUTPUT_DIR,"matches.csv")
        if not os.path.exists(csv_path):
            print("❗ Run matching first"); return
        ...
        print(f"📸 Exporting {len(m)} images → {outdir}")
